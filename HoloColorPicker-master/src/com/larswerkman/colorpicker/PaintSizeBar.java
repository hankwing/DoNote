...
public class PaintSizeBar extends View {
	/*
	...
	public PaintSizeBar(Context context) {
		super(context);
		mContext = context;
		init(null, 0);
	}

	public PaintSizeBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init(attrs, 0);
	}

	public PaintSizeBar(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		mPosToOpacFactorSize = dip2px(mContext, 30) / ((float) mBarLength);
		mOpacToPosFactorSize = ((float) mBarLength) / dip2px(mContext, 30);
	}
	...
	public void setSize(int size) {
		mBarPointerPosition = Math.round((mOpacToPosFactorSize * size))
				+ mBarPointerHaloRadius;
		calculateColor(mBarPointerPosition);
		mBarPointerPaint.setColor(mColor);
		mPicker.setPaintSize(size);
		invalidate();
	}
	...
	public int getSize() {
		int size = Math
				.round((mPosToOpacFactorSize * (mBarPointerPosition - mBarPointerHaloRadius)));
		if (size < dip2px(mContext, 3))
		{
			return dip2px(mContext, 3);
		} else if (size > dip2px(mContext, 29))
		{
			return dip2px(mContext, 29);
		} else
		{
			return size;
		}
